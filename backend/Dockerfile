...
RUN a2enmod rewrite
#@1COPY --from=gcr.io/PROJECT_ID123/secret ./REPONAME123/.env /var/www/html
RUN apt update -y && apt install zip -y && curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer
RUN composer update symfony/flex --no-plugins --no-scripts

#@2RUN composer dump-env prod
#@1COPY --from=gcr.io/PROJECT_ID123/secret ./REPONAME123/.htaccess /var/www/html/public
COPY --from=gcr.io/PROJECT_ID123/secret ./REPONAME123/jwt/ /var/www/html/config/jwt
